...
1. **Product marketing context** (required) — anything that describes the product's positioning, value props, target audience, and key messaging. This is what the generated headlines will draw from. This could be:
   ...
   - A link to their website or homepage (Codex will extract positioning from it)
...
This lets Codex create spreadsheets, read headline data, and write rows from the terminal. Skip this section if you're providing copy guidelines as plain text or a doc.
...
> **Pro tip:** If you use Codex, you can now just say "create a Google spreadsheet called X" and Codex will use the CLI for you — no need to memorize these commands.

---

### 2. Figma MCP plugin: Codex Talk to Figma

This plugin lets Codex read and manipulate Figma files in real time through a WebSocket connection.

#### Step 1: Install the MCP package

```bash
npx Codex-talk-to-figma-mcp
```
...
4. Navigate to the downloaded `Codex-talk-to-figma-mcp` folder → go into `src/` → select `manifest.json`

> **Important:** Make sure you select the `manifest.json` inside the `src/` folder, not the root of the project.

#### Step 3: Configure the MCP for Codex

Copy the MCP configuration from the plugin's README and tell Codex to install it:

```
Install this MCP: { "mcpServers": { "Codex-talk-to-figma": { "command": "npx", "args": ["Codex-talk-to-figma-mcp"] } } }
```

Restart Codex, then check your MCP list to confirm "Codex-talk-to-figma" appears and is connected.

#### Step 4: Connect to Figma

1. Open your Figma file with the ad designs
2. Go to **Plugins** → **Codex Talk to Figma** → **Run**
3. The plugin panel shows a **channel name** (e.g., `abc123`)
4. Copy this channel name and give it to Codex so it can connect
...
> **Tip:** When using the CLI from Codex, special characters in headline text can cause shell escaping issues. If `gws` append commands fail, try writing headlines one at a time or use simpler punctuation.
